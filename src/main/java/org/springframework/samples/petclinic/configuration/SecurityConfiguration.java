...
public class SecurityConfiguration extends WebSecurityConfigurerAdapter {
	...
	protected void configure(final HttpSecurity http) throws Exception {
		http.authorizeRequests().antMatchers("/resources/**", "/webjars/**", "/h2-console/**").permitAll().antMatchers(HttpMethod.GET, "/", "/oups").permitAll().antMatchers("/users/new").permitAll().antMatchers("/admin/**").hasAnyAuthority("admin")
			...
			.antMatchers("/myCauses").hasAnyAuthority("admin", "owner", "veterinarian").antMatchers("/causes/PendingCauses").hasAnyAuthority("admin").antMatchers("/causes/PendingCauses/**").hasAnyAuthority("admin").antMatchers("/vets/**").authenticated()
			.antMatchers("/request/**").authenticated().antMatchers("/rooms/**").hasAnyAuthority("admin", "owner", "veterinarian", "sitter").antMatchers("/vets/create").hasAnyAuthority("admin").antMatchers("/vets/visit").hasAnyAuthority("veterinarian")
			.antMatchers("/vet/{vetId}/diagnosis").hasAnyAuthority("veterinarian").antMatchers("/diagnosis/myDiagnosis").hasAnyAuthority("owner").antMatchers("/visits").permitAll().antMatchers("/vets/**").authenticated().anyRequest().denyAll().and()
			.formLogin()
			/* .loginPage("/login") */
